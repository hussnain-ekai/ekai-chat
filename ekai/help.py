...
from ekai import __version__, utils
from ekai.dump import dump  # noqa: F401
from ekai.help_pats import exclude_website_pats
...
def get_package_files():
    for path in importlib_resources.files("ekai.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    return f"https://ekai.chat/{url_path}"
...
def get_index():
    ...
    dname = Path.home() / ".ekai" / "caches" / ("help." + __version__)
    ...
    if index is None:
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("ekai.website")
                .joinpath(fname)
...
# Attribution
ekai-chat is based on [Aider](https://github.com/Aider-AI/aider), an open-source AI pair programming tool created by Aider AI (https://aider.chat).
