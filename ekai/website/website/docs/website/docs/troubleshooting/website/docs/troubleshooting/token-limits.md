...
ekai will report an error **if a model responds** indicating that
...
For more info: https://ekai.chat/docs/token-limits.html
```

{: .note }
ekai never *enforces* token limits, it only *reports* token limit errors
from the API provider.
The token counts that ekai reports are *estimates*.
...
It's best to only add the files that ekai will need to *edit*
...
as 4k tokens.
If you ask ekai to make a large change that affects a lot
...
ekai has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://ekai.chat/docs/llms.html).
For serving local models, 
[Ollama](https://ekai.chat/docs/llms/ollama.html) is known to work well with ekai.

Try using ekai without an API proxy server
