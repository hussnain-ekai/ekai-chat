...
ekai no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://ekai.chat/docs/repomap.html).
...
To address these issues, `ekai` now
...
[chat transcript](https://ekai.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [ekai](https://ekai.chat/docs/install.html).
  - Install universal ctags.
  - Run `ekai` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
The latest version of `ekai` sends a **repo map** to GPT along with
...
[main.py](https://github.com/ekai-AI/ekai/blob/main/ekai/main.py)
and
[io.py](https://github.com/ekai-AI/ekai/blob/main/ekai/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `ekai` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `ekai` work
