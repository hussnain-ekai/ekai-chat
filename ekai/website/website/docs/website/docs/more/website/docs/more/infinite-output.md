...
parent: More info
nav_order: 480
description: ekai can handle "infinite output" from models that support prefill.
---
...
ekai is able to work around this limit with models that support
"prefilling" the assistant response.
When you use ekai with a model that supports prefill, you will see
...
When ekai is collecting code edits from a model and
it hits the output token limit,
ekai simply initiates another LLM request with the partial
response prefilled.
...
ekai supports "infinite output" for models that support "prefill",
such as:
