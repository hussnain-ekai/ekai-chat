...
ekai can connect to models provided by Amazon Bedrock.
You will need to have an AWS account with access to the Bedrock service.

To configure ekai to use the Amazon Bedrock API, you need to set up your AWS credentials.
...
To use ekai installed via `pipx` with AWS Bedrock, you must add the `boto3` dependency to ekai's virtual environment by running

```bash
pipx inject ekai-chat boto3
```

You must install `boto3` dependency to ekai's virtual environment installed via one-liner or uv by running

```bash
uv tool run --from ekai-chat pip install boto3
```


## Running ekai with Bedrock

Once your AWS credentials are set up, you can run ekai with the `--model` command line switch, specifying the Bedrock model you want to use:

```bash
ekai --model bedrock/anthropic.claude-3-5-sonnet-20240620-v1:0
```

Sometimes it seems to help if you prefix the model name with "us.":

```bash
ekai --model bedrock/us.anthropic.claude-3-5-sonnet-20240620-v1:0
```
...
```bash
ekai --list-models bedrock/
```

Make sure you have access to these models in your AWS account before attempting to use them with ekai.
