...
ekai now asks GPT-4 Turbo to use
...
ekai's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using ekai's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **ekai's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using ekai's existing "SEARCH/REPLACE block" edit format.
- **ekai's new unified diff edit format raised June GPT-4's score to 59%**. 
...
ekai's new unified diff editing format
...
numerous variations on ekai's existing editing formats,
...
ekai's new editing format and refactoring benchmark.
...
The design and implementation of ekai's new unified diff editing format
...
ekai's [previous benchmark results](https://ekai.chat/docs/benchmarks.html) made
...
ekai tells GPT not to include line numbers,
...
ekai's system prompt encourages
...
When a patch fails, ekai needs to ask GPT for a corrected version of the diff.
...
ekai tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, ekai uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on ekai's original Exercism benchmark.

## Refactoring benchmark

ekai has long used a
[benchmark suite based on 133 Exercism python exercises](https://ekai.chat/2023/07/02/benchmarks.html).
...
[9 popular open source python repositories](https://github.com/ekai-AI/refactor-benchmark)
...
A [simple python AST scanning script](https://github.com/ekai-AI/ekai/blob/main/benchmark/refactor_tools.py)
...
[benchmark suite that provokes, detects and quantifies GPT coding laziness](https://github.com/ekai-AI/refactor-benchmark).
...
ekai's new unified diff format seems
...
incorporated into ekai provide
...
ekai's simple, high level style of unified diffs.
