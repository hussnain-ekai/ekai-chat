...
nav_order: 20
description: Using a .env file to store LLM API keys for ekai.
---

# Config with .env

You can use a `.env` file to store API keys and other settings for the
models you use with ekai.
You can also set many general ekai options
in the `.env` file.

ekai will look for a `.env` file in these locations:
...
can also
[download from GitHub](https://github.com/ekai-AI/ekai/blob/main/ekai/website/assets/sample.env).

<!--[[[cog
from ekai.args import get_sample_dotenv
from pathlib import Path
text=get_sample_dotenv()
Path("ekai/website/assets/sample.env").write_text(text)
cog.outl("```")
cog.out(text)
cog.outl("```")
]]]-->
```
##########################################################
# Sample ekai .env file.
# Place at the root of your git repo.
# Or use `ekai --env <fname>` to specify.
##########################################################
...
# See https://ekai.chat/docs/llms.html for details.
...
#AIDER_MODEL_SETTINGS_FILE=.ekai.model.settings.yml

## Specify a file with context window and costs for unknown models
#AIDER_MODEL_METADATA_FILE=.ekai.model.metadata.json
...
#AIDER_INPUT_HISTORY_FILE=.ekai.input.history

## Specify the chat history file (default: .ekai.chat.history.md)
#AIDER_CHAT_HISTORY_FILE=.ekai.chat.history.md
