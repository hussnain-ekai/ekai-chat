...
In most cases, you can safely ignore ekai's warning about unknown context
window size and model costs.

{: .note }
ekai never *enforces* token limits, it only *reports* token limit errors
from the API provider.
You probably don't need to
configure ekai with the proper token limits
for unusual models.

But, you can register context window limits and costs for models that aren't known
to ekai. Create a `.ekai.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch ekai.
...
in the `.ekai.model.metadata.json` file.
...
ekai has a number of settings that control how it works with
different models.
...
a model that ekai doesn't know about.


### Configuration file locations

You can override or add settings for any model by creating a `.ekai.model.settings.yml` file in one of these locations:
...
You can use the special model name `ekai/extra_params` to define 
...
`ekai/extra_params` settings taking precedence for any direct conflicts.
...
[models.py](https://github.com/ekai-AI/ekai/blob/main/ekai/models.py)
file for more details about all of the model setting that ekai supports.
...
<!--[[[cog
from ekai.models import get_model_settings_as_yaml
cog.out("```yaml\n")
