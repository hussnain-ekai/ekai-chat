...
from ekai.mdstream import MarkdownStream

from .dump import dump  # noqa: F401
from .utils import is_image_file

# Constants
NOTIFICATION_MESSAGE = "ekai is waiting for your input"
...
class InputOutput:
    ...
    def __init__(
        ...
        self.append_chat_history(f"\n# ekai chat started at {current_time}\n\n")
    ...
    def get_default_notification_command(self):
        ...
        if system == "Darwin":  # macOS
            ...
            if shutil.which("terminal-notifier"):
                return f"terminal-notifier -title 'ekai' -message '{NOTIFICATION_MESSAGE}'"
            # Fall back to osascript
            return (
                f'osascript -e \'display notification "{NOTIFICATION_MESSAGE}" with title "ekai"\''
            )
        elif system == "Linux":
            ...
            for cmd in ["notify-send", "zenity"]:
                if shutil.which(cmd):
                    if cmd == "notify-send":
                        return f"notify-send 'ekai' '{NOTIFICATION_MESSAGE}'"
                    elif cmd == "zenity":
