...
import ekai
from ekai import utils
from ekai.dump import dump  # noqa: F401

VERSION_CHECK_FNAME = Path.home() / ".ekai" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of ekai from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of ekai from the main branch?",
        ["git+https://github.com/ekai-AI/ekai.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of ekai from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer ekai version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of ekai?"
...
"""
    ...
    if success:
        io.tool_output("Re-run ekai to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/ekai-chat/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = ekai.__version__
