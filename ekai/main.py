...
from ekai import __version__, models, urls, utils
from ekai.analytics import Analytics
from ekai.args import get_parser
from ekai.coders import Coder
from ekai.coders.base_coder import UnknownEditFormat
from ekai.commands import Commands, SwitchCoder
from ekai.copypaste import ClipboardWatcher
from ekai.format_settings import format_settings, scrub_sensitive_info
from ekai.history import ChatSummary
from ekai.io import InputOutput
from ekai.llm import litellm  # noqa: F401; properly init litellm on launch
from ekai.models import ModelSettings
from ekai.repo import ANY_GIT_ERROR, GitRepo
from ekai.report import report_uncaught_exceptions
from ekai.versioncheck import check_version, install_from_main_branch, install_upgrade
from ekai.watch import FileWatcher
...
def setup_git(git_root, io):
    ...
    if git_root:
        ...
        io.tool_warning(
            "You should probably run ekai in your project's directory, not your home dir."
        )
        return
    elif cwd and io.confirm_ask(
        "No git repo found, create one to track ekai's changes (recommended)?"
    ):
...
def check_gitignore(git_root, io, ask=True):
    ...
    try:
        ...
        if not repo.ignored(".ekai"):
            patterns_to_add.append(".ekai*")
...
def check_streamlit_install(io):
    return utils.check_pip_install_extra(
        io,
        "streamlit",
        "You need to install the ekai browser feature",
        ["ekai-chat[browser]"],
    )
...
def write_streamlit_credentials():
    ...
    # See https://github.com/ekai-AI/ekai/issues/772
...
def launch_gui(args):
    ...
    # https://github.com/ekai-AI/ekai/issues/2193
...
def register_models(git_root, model_settings_fname, io, verbose=False):
    model_settings_files = generate_search_path_list(
        ".ekai.model.settings.yml", git_root, model_settings_fname
    )
    ...
    try:
        ...
        io.tool_error(f"Error loading ekai model settings: {e}")
        return 1
...
def register_litellm_models(git_root, model_metadata_fname, io, verbose=False):
    ...
    resource_metadata = importlib_resources.files("ekai.resources").joinpath("model-metadata.json")
    model_metadata_files.append(str(resource_metadata))

    model_metadata_files += generate_search_path_list(
        ".ekai.model.metadata.json", git_root, model_metadata_fname
    )
...
def sanity_check_repo(repo, io):
    ...
    if bad_ver:
        io.tool_error("ekai only works with git repos with version number 1 or 2.")
        io.tool_output("You may be able to convert your repo: git update-index --index-version=2")
        io.tool_output("Or run ekai --no-git to proceed without using git.")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    if args.analytics is not False:
        if analytics.need_to_ask(args.analytics):
            ...
            disable = not io.confirm_ask(
                "Allow collection of anonymous analytics to help improve ekai?"
            )
    ...
    if args.apply:
        ...
        # For testing #2879
        # from ekai.coders.base_coder import all_fences
...
def is_first_run_of_new_version(io, verbose=False):
    ...
    installs_file = Path.home() / ".ekai" / "installs.json"
    ...
    try:
        if is_first_run:
            ...
            try:
                ...
                io.tool_error(str(err))
                io.tool_output("Error loading required imports. Did you install ekai properly?")
